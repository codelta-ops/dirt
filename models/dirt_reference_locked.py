...
def config_ws(ws, config_dict):
    if not os.path.exists(ws):
        os.makedirs(ws)
    if not os.path.exists(ws + '/snapshot'):
    ...
    def train(self, mode=1):
        ...
        if mode == 0:
            ...
            if mode == 1:
                ...
                # DIRT stage2 follows the paper protocol: freeze IRT and only
                # update the sequential transition module and decoder.
                for param in self.parameters():
...
def reset_metric_logs(ws):
    # Reset logs at the beginning of a fresh run so best-epoch selection only
    # uses the current experiment's validation records.
    for log_name in ['results.txt', 'metrics.jsonl']:
        log_path = f'{ws}/{log_name}'
        with open(log_path, 'w', encoding='utf8'):
            pass


def append_metric_record(ws, dtype, stage, epoch, auc, rmse, acc):
    metric_record = {
        'dtype': dtype,
        'stage': int(stage),
        'epoch': int(epoch),
    ...
    with open(f'{ws}/metrics.jsonl', 'a', encoding='utf8') as o_f:
        o_f.write(json.dumps(metric_record, ensure_ascii=True) + '\n')


def find_best_epoch(ws, stage, dtype='validation', key='auc'):
    metrics_path = f'{ws}/metrics.jsonl'
    ...
    if not metric_records:
        raise ValueError(f'No metric records found for dtype={dtype}, stage={stage}.')
    if key not in metric_records[0]:
        raise ValueError(f'Key "{key}" is not present in metrics.jsonl records.')

    best_record = max(metric_records, key=lambda record: (record[key], -record['epoch']))
    return best_record['epoch'], float(best_record[key])


def compute_prediction_loss(loss_function, pred_out, corrs, log_lens, batch_len, batch_size, max_log):
    pred_out = torch.clamp(pred_out, min=EPSILON, max=1 - EPSILON)
    ...
    pred_loss_list = pred_loss_list.view(batch_size, max_log - 1)
    pred_loss = 0.
    for stu_i in range(batch_len):
        pred_loss = pred_loss + pred_loss_list[stu_i][:log_lens[stu_i] - 1].sum()
...
def train_stage1(ws, device='cpu', lr=0.002, n_epochs=10):
    ...
    for epoch in range(n_epochs):
        ...
        while not data_loader.is_end():
            ...
            # Keep the original NLL objective, but clamp probabilities before
            # log(.) to avoid numerical instability from log(0).
            pred_loss = compute_prediction_loss(
                loss_function, pred_out, corrs, log_lens, batch_len, batch_size, ws_config_dict['max_log']
            )
            loss = pred_loss
            loss.backward()
            optimizer.step()
...
def train_stage2(ws, stage1_epoch=5, n_epochs=8, device='cpu', lr=0.002):
    ...
    # read the configuration of the work space
    ws_config_dict = read_ws_config(ws)
    if ws_config_dict['attr_idx'] not in [3, 4]:
        raise ValueError('stage2 is only applicable to DIRT_3 / DIRT_4 (attr_idx in [3, 4]).')
    batch_size = ws_config_dict['batch_size']
    # initialize model
    model = Model(
        stu_ho_dim_lcl=ws_config_dict['stu_ho_dim'],
...
def val_test(ws, model, epoch_i, stage, dtype='validation'):
    ...
    pred_acc = (pred_binary == pred_label_all).mean()
    print('[{}] stage={}, epoch={}: auc={}, rmse={}, acc={}'.format(dtype, stage, epoch_i, pred_auc, pred_rmse, pred_acc))
    with open(f'{ws}/results.txt', 'a', encoding='utf8') as o_f:
        o_f.write('[{}] stage={}, epoch={}: auc={}, rmse={}, acc={}\n'.format(dtype, stage, epoch_i, pred_auc, pred_rmse, pred_acc))
    # Write structured metrics so best checkpoint selection depends only on
    # validation records instead of scanning test outputs.
    append_metric_record(ws, dtype, stage, epoch_i, pred_auc, pred_rmse, pred_acc)
    return {'auc': pred_auc, 'rmse': pred_rmse, 'acc': pred_acc}


def test(ws, epoch_model=None, stage=2, device='cpu'):
    if epoch_model is None:
        raise ValueError('test() requires an explicit epoch_model so testing only runs once on the selected best checkpoint.')
    # read the configuration of the work space
    ws_config_dict = read_ws_config(ws)
    # initialize model
    model = Model(
    ...
    model = model.to(device)
    return val_test(ws=ws, model=model, epoch_i=epoch, stage=stage, dtype='test')


def run_dirt_experiment(ws, device, lr):
    ws_config_dict = read_ws_config(ws)
    attr_idx = ws_config_dict['attr_idx']
    if attr_idx not in [1, 2, 3, 4]:
        raise ValueError('attr_idx must be one of [1, 2, 3, 4] for DIRT_1 ~ DIRT_4.')

    reset_metric_logs(ws)

    # Align the training/testing schedule with the DIRT paper:
    # DIRT_1/2 use only stage1, while DIRT_3/4 use best-stage1 initialization
    # and select the final model from stage2 validation metrics.
    train_stage1(ws, device=device, lr=lr)
    best_stage1_epoch, best_stage1_auc = find_best_epoch(ws, stage=1, dtype='validation', key='auc')
    print('best stage1 epoch={}, auc={}'.format(best_stage1_epoch, best_stage1_auc))

    if attr_idx in [1, 2]:
        test(ws, epoch_model=best_stage1_epoch, stage=1, device=device)
        return

    train_stage2(ws, stage1_epoch=best_stage1_epoch, n_epochs=5, device=device, lr=lr)
    best_stage2_epoch, best_stage2_auc = find_best_epoch(ws, stage=2, dtype='validation', key='auc')
    print('best stage2 epoch={}, auc={}'.format(best_stage2_epoch, best_stage2_auc))
    test(ws, epoch_model=best_stage2_epoch, stage=2, device=device)


def run_all_dirt_experiments(ws_root, device, lr, base_ws_config):
    # Run DIRT_1 ~ DIRT_4 sequentially with separate workspaces so each model
    # keeps its own snapshots and logs.
    for attr_idx in [1, 2, 3, 4]:
        ws = f'{ws_root}/DIRT_{attr_idx}'
        ws_config_dict = dict(base_ws_config)
        ws_config_dict['attr_idx'] = attr_idx
        config_ws(ws, ws_config_dict)
        print('========== Running DIRT_{} at {} =========='.format(attr_idx, ws))
        run_dirt_experiment(ws, device=device, lr=lr)
